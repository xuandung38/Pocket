...
const EmojiStudio = () => {
  ...
  // Threshold for distinguishing scroll vs hold
    ...
    try {
      ...
      if (!recentEmojis.includes(emoji)) {
        ...
        try {
          ...
          console.error("Error saving recent emojis:", storageError);
        }
      }
    } catch (error) {
  ...
  const handleMouseUp = (emoji, emojiId) => {
    if (isTouching.current || hasSentRef.current) return;
  ...
  const handleTouchEnd = (e, emoji, emojiId) => {
    ...
    if (hasSentRef.current) return; // already sent via hold
  ...
  // Start hold effect — power 0→100 over 5s (50ms per tick)
